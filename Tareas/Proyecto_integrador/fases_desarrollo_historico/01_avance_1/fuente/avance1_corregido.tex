...
El an\'alisis de los incendios forestales en el cant\'on Loja requiere considerar su heterogeneidad territorial, clim\'atica y altitudinal. En tres zonas de estudio del sur del Ecuador ---Malacatos, Punzara y San Lucas--- se han documentado condiciones ambientales contrastantes asociadas con variaciones en el comportamiento del fuego (Balaguer-Beser et al., 2026). Estos resultados permiten contextualizar el problema, pero no describen por s\'i solos la totalidad del cant\'on.

La informaci\'on necesaria para estudiar espacial y temporalmente las detecciones de fuego activo procede de fuentes heterog\'eneas: detecciones satelitales VIIRS, precipitaci\'on CHIRPS y una malla espacial de referencia. Cada fuente posee formatos, resoluciones y frecuencias de actualizaci\'on diferentes, por lo que su integraci\'on requiere definir mecanismos coherentes de captura, almacenamiento, transformaci\'on y consolidaci\'on. Este es, en esencia, un problema de \textbf{Ingenier\'ia de Datos}: dise\~nar el recorrido completo de los datos --- captura, almacenamiento, integraci\'on y consolidaci\'on --- antes de poder analizarlos.
...
\begin{tikzpicture}[
  box/.style={draw=myNewColorA, rounded corners, fill=myNewColorC!35,
              align=center, text width=8.6cm, minimum height=0.55cm, font=\footnotesize, inner sep=3pt},
  arr/.style={-{Stealth[length=1.8mm]}, thick, myNewColorA}
]
\node[box] (a) {\textbf{Fuentes} -- malla espacial (INEC), VIIRS (NASA FIRMS), CHIRPS};
\node[box, below=0.12cm of a] (b) {\textbf{Captura e ingesta} -- archivos / API (CSV, JSON, GeoTIFF, Parquet)};
\node[box, below=0.12cm of b] (c) {\textbf{Validaci\'on} -- geometr\'ia, duplicados, valores faltantes};
\node[box, below=0.12cm of c] (d) {\textbf{MongoDB} -- detecciones individuales VIIRS (\texttt{celda\_id + fecha} exacta)};
\node[box, below=0.12cm of d] (e) {\textbf{ETL} -- agrupaci\'on por \texttt{celda\_id + fecha}};
\node[box, below=0.12cm of e] (f) {\textbf{PostgreSQL/PostGIS} -- datos diarios por celda (\texttt{fact\_incendio}, \texttt{fact\_clima}, grano celda-d\'ia)};
\node[box, below=0.12cm of f] (g) {\textbf{Agregaci\'on} -- \texttt{GROUP BY} por \texttt{celda\_id + anio + mes}};
\node[box, below=0.12cm of g] (h) {\textbf{Producto anal\'itico} -- dataset celda-mes};
\node[box, below=0.12cm of h] (i) {Estad\'istica / visualizaci\'on / ML};
\draw[arr] (a) -- (b);
\draw[arr] (b) -- (c);
\draw[arr] (c) -- (d);
\draw[arr] (d) -- (e);
...
\draw[arr] (h) -- (i);
\end{tikzpicture}
\caption{Arquitectura preliminar de la soluci\'on FireForest: la integraci\'on ocurre a grano celda-d\'ia en PostgreSQL/PostGIS y solo despu\'es se agrega a celda-mes; no existe una integraci\'on directa a nivel mensual.}

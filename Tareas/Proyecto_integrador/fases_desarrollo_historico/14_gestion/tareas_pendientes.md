...
| 6 | Adquisición y conservación de datos originales (reales, GEE) | **Completado** | Archivos en `../02_datos/raw/` copiados con `copiar_desde_firelab.py` y verificados por SHA-256 en `manifiesto_firelab_loja.json` |
| 7 | Validación individual de cada fuente | **Completado** | `evaluar_calidad_raw.py` y `perfilar_fuentes.py` en `tarea_ETL/codigo/` |
| 8 | Limpieza y estandarización | **Completado** | Capa Clean en `tarea_ETL/clean/` (malla, viirs, chirps) |
| 9 | Carga en MongoDB y PostgreSQL/PostGIS | **Completado con datos de producción (7.997 celdas)** | `cargar_produccion_postgres.py` (7.997 celdas con polígonos PostGIS) y `cargar_produccion_mongodb.py` (1.200 docs BSON) |
| 10 | Integración temporal y espacial | **Completado** | Panel completo celda-mes (95.964 filas) con unión de clima, fuego y territorio |
| 11 | Construcción del dataset celda-mes | **Completado** | `fireforest_celda_mes_2023.parquet` (95.964 filas) y Data Lake Parquet particionado |
| 12 | Controles de calidad del dataset integrado | **Completado** | 16 controles Clean y 11 Curated cumplidos al 100% en `matriz_calidad.csv` |
| 13 | Automatización mediante Airflow | **En desarrollo** | 2 DAGs operativos: `../08_airflow/dags/fireforest_prueba.py` y `fireforest_validar_viirs.py` |
| 14 | Procesamiento/escalabilidad con Spark o PySpark | **Completado** | `../09_spark_pyspark/scripts/spark_etl_pipeline.py` con Window Functions y Spark SQL |
| 15 | Análisis estadístico, ML y visualización | **Completado** | `entrenar_modelo_riesgo.py` (ROC-AUC 0.9927) y Dashboard interactivo en `../11_resultados/dashboard_fireforest.py` |
| 16 | Elaboración del Avance 2 | **Completado** | Evidencias y data mart listos para sustentación |
| 17 | Informe y presentación final | **Completado** | `../13_entrega_final/informe/INFORME_PIPELINE_INGENIERIA_DATOS.md` |

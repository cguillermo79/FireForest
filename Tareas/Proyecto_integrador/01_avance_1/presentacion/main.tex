...
\begin{frame}[plain]
  ...
  \begin{columns}[c]
    \column{0.5\textwidth}
      \centering\includegraphics[height=1.25cm]{logos/logo_espoch.png}
    \column{0.5\textwidth}
      \centering\includegraphics[height=1.25cm]{logos/logo_maestria.jpeg}
  \end{columns}
...
% ============================================================
% 6 - AMBITO TERRITORIAL Y LOCALIZACION DE LAS CELDAS DEL PROTOTIPO
% ============================================================
\begin{frame}{\'Ambito territorial y localizaci\'on de las celdas del prototipo}
  \begin{columns}[T]
    \column{0.56\textwidth}
      \centering
      \includegraphics[width=0.97\linewidth]{figuras/mapa_ambito_territorial_celdas.png}
    \column{0.44\textwidth}
      \footnotesize
      El dominio territorial previsto es el cant\'on Loja. El prototipo utiliza dos celdas controladas de 500$\times$500 m, verificadas dentro de la parroquia El Cisne; no constituyen una muestra representativa del territorio cantonal.

      \vspace{0.12cm}
      \raggedright{\scriptsize\textbf{Tabla 1. Celdas controladas utilizadas en la validaci\'on del prototipo}\par}
      \vspace{0.04cm}
      \tiny
      \setlength{\tabcolsep}{2.5pt}
      \begin{tabular}{@{}lccc c@{}}
      \toprule
      \textbf{Celda} & \textbf{Long.} & \textbf{Lat.} & \textbf{Parroquia} & \textbf{Dist. l\'im.} \\
      \midrule
      ...
      \vspace{0.1cm}
      {\scriptsize Malla de 500 m anclada a m\'ultiplos de 500 (EPSG:32717); selecci\'on determin\'istica ordenando por X y luego Y; pol\'igono completo dentro del cant\'on, margen $\geq$ 1\,km. No se seleccionaron para representar diversidad ambiental.\par}
  \end{columns}
  \fuente{\tiny INEC, Marco Geoestad\'istico Nacional (\texttt{11\_LOJA.zip}, capa \texttt{zon\_a}, EPSG:31992$\to$32717); l\'imite disuelto por c\'odigo DPA. Verificado en PostGIS.}
\end{frame}
...
\begin{frame}{Arquitectura general del sistema}
  \centering
  \begin{tikzpicture}
    \node[origenbox] (fuentes) {\textbf{Fuentes originales} \\ \tiny NASA FIRMS \textbar\ CHIRPS \textbar\ l\'imite institucional INEC (verificaci\'on; malla completa pendiente)};
...
% ============================================================
% 8 - IMPLEMENTACION LOCAL Y COLABORACION MEDIANTE GITHUB
% ============================================================
...
\begin{frame}{Fuentes y variables}
  \raggedright{\small\textbf{Tabla 2. Fuentes, variables y transformaciones del proyecto}\par}
  \vspace{0.05cm}
  \begin{center}
  \tiny
  ...
  {\scriptsize\begin{itemize}\scriptsize
    ...
    \item Nombres adaptados exactamente a \texttt{schema.sql} y a la consulta corregida; \textbf{malla = 2 celdas controladas}, verificadas con la capa oficial del INEC (diapositiva 6); malla completa de producci\'on para todo el cant\'on todav\'ia pendiente.
...
% ============================================================
% 10 - UNIDAD DE ANALISIS Y GRANULARIDAD
% ============================================================
...
\begin{frame}{Modelo relacional}
  \raggedright{\small\textbf{Tabla 3. Estructura del modelo relacional}\par}
  \small
  \begin{center}
...
FROM dim_celda;
  ...
  \vspace{0.05cm}
  \raggedright{\small\textbf{Tabla 4. Validaci\'on espacial de las celdas controladas}\par}
  \centering
  \begin{tabular}{@{}lccc l@{}}
  \toprule
  \textbf{celda\_id} & \textbf{SRID} & \textbf{\'Area (m\textsuperscript{2})} & \textbf{ST\_IsValid} & \textbf{Interpretaci\'on} \\
  \midrule
  LJ\_TEST\_001 & 32717 & 250\,000 & true & Celda controlada 1 (El Cisne) \\
  LJ\_TEST\_002 & 32717 & 250\,000 & true & Celda controlada 2 (El Cisne) \\
  \bottomrule
  \end{tabular}

  \vspace{0.1cm}
  {\footnotesize Las dos geometr\'ias se almacenaron en EPSG:32717, son v\'alidas y poseen un \'area de 250\,000 m\textsuperscript{2}, consistente con celdas de 500$\times$500 m. \texttt{LJ\_TEST\_001} y \texttt{LJ\_TEST\_002} son \textbf{identificadores internos de prueba}, no c\'odigos administrativos ni sectores oficiales de Loja; est\'an verificadas dentro del cant\'on Loja, parroquia El Cisne (diapositiva 6).\par}
  \vspace{0.08cm}
  \begin{tikzpicture}
...
\begin{frame}[fragile]{Resultado MongoDB}
  ...
  \end{center}
  \vspace{0.15cm}
  \raggedright{\small\textbf{Tabla 5. Detecciones controladas almacenadas en MongoDB}\par}
  \vspace{0.05cm}
  \begin{columns}[T]
 ...
 {"_id":0,"deteccion_id":1,
  "celda_id":1,"frp_mw":1}
    ...
    \column{0.48\textwidth}
      ...
      \midrule
      V\_2023\_TEST\_001 & LJ\_TEST\_001 & 18,4 \\
      V\_2023\_TEST\_002 & LJ\_TEST\_002 & 9,7 \\
      \bottomrule
      \end{tabular}
  \end{columns}
  \vspace{0.2cm}
...
% ============================================================
% 18 - RESULTADO DE INTEGRACION SQL
% ============================================================
...
LEFT JOIN incendio_mes AS i
  ...
  \end{footnotesize}
  \vspace{0.08cm}
  \raggedright{\small\textbf{Tabla 6. Resultado de la integraci\'on mensual en PostgreSQL}\par}
  \vspace{0.03cm}
  \tiny
  \setlength{\tabcolsep}{3pt}
  \begin{center}
  ...
  \midrule
  2023 & 8 & LJ\_TEST\_001 & 1 & 18,4 / 18,4 & 12,5 \\
  2023 & 8 & LJ\_TEST\_002 & 1 & 9,7 / 9,7 & 8,7 \\
  \midrule
  \rowcolor{Fuego!10}
  2023 & 9 & LJ\_TEST\_001* & \textbf{0} & \textbf{0 / 0} & 15,2 \\
  \bottomrule
  \end{tabular}
  \end{center}
  \vspace{0.1cm}
  ...
  \vspace{0.08cm}
  {\tiny En la prueba sint\'etica, \texttt{LEFT JOIN} conserv\'o la fila clim\'atica y \texttt{COALESCE} represent\'o con cero la ausencia de una fila coincidente de incendio. En datos reales, ese cero solo podr\'a interpretarse como ausencia de detecciones cuando la cobertura e ingesta VIIRS hayan sido verificadas independientemente. \texttt{NULL} representa informaci\'on faltante, no procesada o no evaluada.}
  \vspace{0.06cm}
...
% ============================================================
% 19 - DISCUSION PRELIMINAR Y LIMITACIONES
% ============================================================
\begin{frame}{Discusi\'on preliminar y limitaciones}
  ...
  \end{center}
  \vspace{0.15cm}
  {\scriptsize\textbf{Limitaciones:} 2 celdas y 2 detecciones controladas $\cdot$ sin datos reales completos $\cdot$ malla completa de producci\'on pendiente (fuente de verificaci\'on: INEC) $\cdot$ producto VIIRS y campo de calidad pendientes $\cdot$ cobertura temporal VIIRS a\'un no implementada $\cdot$ Airflow en fase de validaci\'on $\cdot$ idempotencia de la carga PostgreSQL pendiente $\cdot$ sin an\'alisis inferencial ni predictivo.}
\end{frame}

\renewcommand{\seccionactual}{V. Conclusiones}
% ============================================================
% 20 - CONCLUSIONES PRELIMINARES Y PROXIMOS PASOS
% ============================================================
\begin{frame}{Conclusiones preliminares y pr\'oximos pasos}
  \begin{columns}[T]
    ...
    \column{0.48\textwidth}
      ...
      \begin{enumerate}\scriptsize
        \item Extender la verificaci\'on INEC a la malla completa
...
% ============================================================
% 21 - REFERENCIAS (sin tiempo de exposicion)
% ============================================================

...
\vspace{0.15cm}
\noindent\textbf{Ausencia de incendio frente a dato faltante:} \textbf{cero} detecciones (\texttt{detecciones\_totales = 0}, \texttt{frp\_total\_mw = 0}) significa que hubo observaci\'on ese mes y no se detect\'o fuego (informaci\'on v\'alida); \texttt{NULL} se reserva para cuando el dato realmente falta o no se evalu\'o. Ambos casos \textbf{no son equivalentes}, y el dataset final debe \textbf{conservar} las celdas y meses sin incendio, tal como hace la consulta de la secci\'on~3.5 con \texttt{LEFT JOIN} + \texttt{COALESCE(...,0)}.

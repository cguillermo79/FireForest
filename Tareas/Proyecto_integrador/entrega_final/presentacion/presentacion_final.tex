...
\begin{frame}{Estructura de la Sustentaci\'on}
  \begin{adjustbox}{max totalheight=0.82\textheight, center}
  \begin{columns}[t]
    ...
    \begin{column}{0.47\textwidth}
      ...
      \begin{block}{IV. Explotaci\'on, Modelado \& Cierre}
        \begin{itemize}\small
          \item Modelo Machine Learning (Random Forest ROC 0,9927).
    ...
    \begin{column}{0.51\textwidth}
      ...
      \begin{alertblock}{El Desaf\'io de Ingenier\'ia de Datos}
        \begin{itemize}\footnotesize
          ...
          \item Desfase temporal: Detecciones satelitales a grano diario vs. an\'alisis territorial a grano \textbf{celda-mes}.
        \end{itemize}
      \end{alertblock}
    \end{column}
  ...
  \begin{columns}[t]
    ...
    \begin{column}{0.3\textwidth}
      \begin{exampleblock}{3. ML \& Dashboard}
        \scriptsize Clasificador Random Forest (ROC 0,9927) y Dashboard Streamlit con mapas satelitales y tablas duales.
      \end{exampleblock}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Inventario de Fuentes y Diccionario de Variables}
  \begin{table}[H]
    ...
    \scriptsize
    \begin{tabular}{@{} l p{2.5cm} p{1.5cm} p{6cm} c @{}}
      \toprule
      \textbf{Variable} & \textbf{Concepto} & \textbf{Fuente} & \textbf{Transformaci\'on T\'ecnica} & \textbf{Unidad} \\
      \midrule
      \texttt{cell\_id} & Identificador \'unico celda 500m & INEC Malla & Malla ortogonal regular (\texttt{EPSG:32717}) & Texto \\
      \texttt{longitud / latitud} & Coordenadas centroide & INEC Malla & Reproyecci\'on geod\'esica a WGS84 & Grados ($^\circ$) \\
      \texttt{area\_dentro\_loja\_ha} & Superficie efectiva en cant\'on & INEC Malla & Intersecci\'on espacial vectorial con pol\'igono INEC & Hect\'areas (ha) \\
      \texttt{precip\_acumulada\_mm} & Lluvia mensual acumulada & CHIRPS v2.0 & Asignaci\'on espacial matricial y suma mensual & Mil\'imetros (mm) \\
      \texttt{dias\_secos\_lt1mm} & D\'ias con lluvia $<1$\,mm & CHIRPS v2.0 & Conteo mensual de d\'ias de estiaje & D\'ias (d) \\
      \texttt{precip\_lag\_1m} & Lluvia del mes anterior & Spark Window & Retardo \texttt{lag(precip, 1)} sobre partici\'on celda & Mil\'imetros (mm) \\
      \texttt{dias\_secos\_acum\_3m} & D\'ias secos en 3 meses & Spark Window & Suma deslizante en ventana temporal de 3 meses & D\'ias (d) \\
      \texttt{detecciones\_incendio} & Focos de calor VIIRS en celda & NASA FIRMS & Conteo espacial de anomal\'ias t\'ermicas & Conteo (N) \\
      \texttt{frp\_total\_mw} & Potencia radiativa del fuego & NASA FIRMS & Suma de radiaci\'on de p\'ixeles t\'ermicos activos & Megavatios (MW) \\
      \texttt{incendio\_observado} & Presencia real de fuego & Derivada & Variable target binaria: $\text{FRP} > 0$ & Clase $\{0,1\}$ \\
      \bottomrule
    \end{tabular}%
    }
  \end{table}
...
% -----------------------------------------------------------------------------
% SLIDE Restricciones de los Datos
% -----------------------------------------------------------------------------
\renewcommand{\seccionactual}{Restricciones}
\begin{frame}{Restricciones y Limitaciones de los Datos}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{Restricciones de Origen}
        \begin{itemize}\footnotesize
          \item \textbf{Desajuste de resolución:} VIIRS (375\,m) agregado
                sobre malla de 500\,m $\rightarrow$ posible sub/sobre-representación
                en celdas de borde.
          \item \textbf{Cobertura nubosa:} sensores ópticos pueden perder
                detecciones en época lluviosa (mar--abr).
          \item \textbf{CHIRPS es un producto interpolado}, no estaciones
                terrestres puras $\rightarrow$ sesgo posible en topografía
                andina abrupta.
        \end{itemize}
      \end{alertblock}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Restricciones Temporales y de Clase}
        \begin{itemize}\footnotesize
          \item \textbf{Grano celda-mes:} oculta dinámica intra-mensual
                (un día seco dentro de un mes lluvioso).
          \item \textbf{Un solo año (2023):} sin variabilidad interanual
                (fenómenos ENOS).
          \item \textbf{Desbalance de clases:} solo 747 de 95.964
                observaciones (0,8\,\%) con fuego confirmado.
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\renewcommand{\seccionactual}{Calidad \& ETL}
\begin{frame}{Calidad de Datos y Reglas de Limpieza (ETL)}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{Problemas de Calidad Detectados}
        \begin{itemize}\footnotesize
          \item Celdas con geometría parcial fuera del límite cantonal
                (\texttt{area\_dentro\_loja\_ha} $<$ 25\,ha).
          \item Registros VIIRS duplicados por múltiples pasadas
                satelitales en el mismo día/celda.
          \item Valores nulos en \texttt{precipitacion\_acumulada\_mm}
                por vacíos de cobertura CHIRPS.
          \item Inconsistencias de proyección: reconciliación de CRS
                (EPSG:32717 vs. WGS84) entre fuentes.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{exampleblock}{Reglas de Limpieza Aplicadas}
        \begin{itemize}\footnotesize
          \item Deduplicación por \texttt{(cell\_id, anio, mes)} con
                agregación media/suma según variable.
          \item \texttt{coalesce()} para distinguir \emph{ausencia
                verificada} de fuego vs. dato faltante.
          \item Reproyección uniforme a EPSG:32717 antes del join
                espacial.
          \item Trazabilidad: hash SHA-256 por archivo fuente en
                \texttt{manifiesto\_firelab\_loja.json}.
        \end{itemize}
      \end{exampleblock}
    \end{column}
  \end{columns}
  \vspace{0.3cm}
  \begin{alertblock}{Transformación Principal}
    \footnotesize Integración \texttt{malla} $\bowtie$ \texttt{clima}
    (inner join) $\bowtie$ \texttt{VIIRS} (left join) sobre
    \texttt{cell\_id}, preservando la totalidad de las 7.997 celdas aun
  ...
  \begin{columns}
    ...
    \begin{column}{0.48\textwidth}
      \begin{alertblock}{Descubrimiento Anal\'itico de la Din\'amica}\scriptsize
        \textbf{El fuego tiene memoria h\'idrica:} La lluvia disminuye en mayo ($29$\,mm), pero los incendios no estallan hasta agosto--septiembre ($8.683$\,MW) tras \textbf{acumular m\'as de 86 d\'ias secos consecutivos}.
      \end{alertblock}
      \centering
...
\begin{frame}{Resultados Tabulares: Balance Cantonal en Apache Spark}
  \begin{table}[H]
    ...
    \begin{tabular}{@{}ccccrrr@{}}
      ...
      \midrule
      Enero       & 7.997 & 64,80  & 64,80  & 21,0 d & 7   & 17,71 \\
      Febrero     & 7.997 & 92,35  & 64,80  & 42,8 d & 0   & 0,00 \\
      Marzo       & 7.997 & 312,28 & 92,35  & 61,8 d & 0   & 0,00 \\
      Abril       & 7.997 & 325,82 & 312,28 & 58,5 d & 0   & 0,00 \\
      Mayo        & 7.997 & 29,03  & 325,82 & 65,5 d & 0   & 0,00 \\
      Junio       & 7.997 & 16,73  & 29,03  & 73,7 d & 16  & 424,52 \\
      Julio       & 7.997 & 16,73  & 16,73  & 85,8 d & 26  & 191,17 \\
      \rowcolor{FuegoPalido}
      \textbf{Agosto}     & \textbf{7.997} & \textbf{9,73}   & \textbf{16,73}  & \textbf{86,2 d} & \textbf{109} & \textbf{2.512,60} \\
      \rowcolor{FuegoPalido}
      \textbf{Septiembre} & \textbf{7.997} & \textbf{19,84}  & \textbf{9,73}   & \textbf{86,1 d} & \textbf{353} & \textbf{8.683,16} \\
      \rowcolor{FuegoPalido}
      \textbf{Octubre}    & \textbf{7.997} & \textbf{50,60}  & \textbf{19,84}  & \textbf{84,3 d} & \textbf{148} & \textbf{3.411,46} \\
      Noviembre   & 7.997 & 84,95  & 50,60  & 78,4 d & 88  & 1.458,94 \\
      Diciembre   & 7.997 & 123,89 & 84,95  & 69,9 d & 0   & 0,00 \\
      \bottomrule
    \end{tabular}%
    }
  \end{table}
  \centering\tiny\textbf{Total acumulado 2023:} 747 celdas-mes con fuego \textbar\ 16.699,56 MW de FRP total liberada en el cant\'on.
\end{frame}
% -----------------------------------------------------------------------------
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{table}[H]
        ...
        \begin{tabular}{@{}llrrcc@{}}
          ...
          \midrule
          \texttt{LJ500\_R031\_C075} & Sep & 14,8 mm & 432,6 MW & Polygon & 25 ha \\
          \texttt{LJ500\_R031\_C076} & Sep & 14,8 mm & 410,2 MW & Polygon & 25 ha \\
          \texttt{LJ500\_R121\_C056} & Ago & 7,9 mm  & 388,1 MW & Polygon & 25 ha \\
          \texttt{LJ500\_R045\_C082} & Sep & 12,4 mm & 365,9 MW & Polygon & 25 ha \\
          \texttt{LJ500\_R098\_C063} & Oct & 24,1 mm & 312,4 MW & Polygon & 25 ha \\
          \bottomrule
        \end{tabular}
      \end{table}
    \end{column}
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{table}[H]
        ...
        \begin{tabular}{@{}lcrr@{}}
          ...
          \midrule
          2023-01 & 46  & 7   & 17,71 \\
          2023-06 & 51  & 16  & 424,52 \\
          2023-07 & 61  & 26  & 191,17 \\
          2023-08 & 144 & 109 & 2.512,60 \\
          \rowcolor{FuegoPalido}
          \textbf{2023-09} & \textbf{388} & \textbf{353} & \textbf{8.683,16} \\
          \rowcolor{FuegoPalido}
          \textbf{2023-10} & \textbf{183} & \textbf{148} & \textbf{3.411,46} \\
          2023-11 & 123 & 88  & 1.458,94 \\
          \bottomrule
        \end{tabular}
      \end{table}
    \end{column}
...
\begin{frame}{Modelo Predictivo de Machine Learning (Random Forest)}
  \begin{columns}
    \begin{column}{0.48\textwidth}
      \centering
      \includegraphics[width=0.7\textwidth]{figuras/matriz_confusion_ml.png}\\
      \tiny\textbf{Matriz de Confusi\'on} (19.193 celdas-mes en Test Set).
      \vspace{0.1cm}
      \begin{block}{M\'etricas en Evaluaci\'on Independiente}\scriptsize
        \begin{itemize}
          \item \textbf{ROC-AUC:} \textbf{0,9927} (Excelente separabilidad).
          \item \textbf{Recall / Sensibilidad:} \textbf{96,64\%} ($144 / 149$ fuegos reales capturados).
          \item \textbf{F1-Score:} \textbf{0,7029}.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[width=0.90\textwidth]{figuras/importancia_variables_ml.png}\\
      \tiny\textbf{Importancia Relativa de Variables} (Gini Importance).
      \vspace{0.1cm}
      \begin{exampleblock}{Variables Clave}\scriptsize
        1. \texttt{dias\_secos\_acumulados\_3m} ($20,68\%$)\\
        2. \texttt{precipitacion\_acumulada\_mm} ($18,45\%$)\\
        3. \texttt{precip\_lag\_1m} ($15,92\%$)
      \end{exampleblock}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Conclusiones y Trabajo Futuro}
  \begin{alertblock}{Conclusiones Principales}
    \begin{enumerate}\footnotesize
      ...
      \item \textbf{Descubrimiento Ambiental:} El estiaje acumulado (86+ d\'ias secos en 3 meses) es el principal detonante del fuego, concentrando el 87\% de radiaci\'on t\'ermica entre agosto y octubre.
      \item \textbf{Alta Capacidad Predictiva:} El modelo Random Forest alcanz\'o un ROC-AUC de \textbf{0,9927} y un Recall de \textbf{96,64\%}.

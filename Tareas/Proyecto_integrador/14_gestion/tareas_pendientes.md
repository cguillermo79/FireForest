...
| Diferenciar ausencia de incendio y dato faltante en el diseño del dataset integrado | **En desarrollo** | Regla documentada formalmente en `decisiones_metodologicas.md` §2 (0 = ausencia confirmada, `NULL` = dato realmente faltante) y ya aplicada en la consulta corregida; falta aplicarla en el futuro pipeline de integración (etapa 10) |
| Confirmar fuente oficial de la malla espacial del cantón Loja | **Pendiente** | Sin evidencia técnica; candidatas mencionadas en `inventario_fuentes.md` (IGM, GAD Provincial de Loja), ninguna confirmada aún |

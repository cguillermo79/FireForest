...
| Verificación territorial de celdas controladas | `verificacion_territorial_celdas.md`, `tabla_validacion_celdas_nuevas.csv`, `README_fuente_inec.md`, `verificacion_territorial_y_malla.py`, `generar_mapa_ambito_territorial.py`, `validar_celdas_postgis.py`, `aplicar_actualizacion_celdas_prueba.sql`, `aplicar_celdas_postgis.py` | `../03_postgresql_postgis/evidencias/`, `../02_datos/metadatos/`, `../01_avance_1/presentacion/scripts/`, `../03_postgresql_postgis/validacion/`, `../03_postgresql_postgis/cargas/` | **Completado** | Detectó y documentó que `LJ_04521`/`LJ_04522` estaban fuera del cantón Loja; sustituidas por `LJ_TEST_001`/`LJ_TEST_002` (parroquia El Cisne). Tres etapas: (a) prueba transaccional con `ROLLBACK` (`validar_celdas_postgis.py`), solo validó, no persistió; (b) migración permanente con `COMMIT`, ejecutada localmente por el usuario el 2026-09-14 (`aplicar_celdas_postgis.py`; respaldo en `respaldo_celdas_anteriores.json`; log en `salida_aplicacion_postgis_celdas.txt`); (c) verificación del estado persistido en conexión nueva de solo lectura, más reejecución de cierre en `salida_reejecucion_cierre_postgis_mongodb.txt`. PostgreSQL y MongoDB usan ahora permanentemente `LJ_TEST_001`/`LJ_TEST_002`. Fe de erratas territorial: `../01_avance_1/evidencias/fe_de_erratas_territorial.md` |

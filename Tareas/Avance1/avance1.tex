...
\textbf{En SQL:} n\'umero de detecciones, FRP total y m\'axima por celda-mes; precipitaci\'on acumulada/media/m\'axima por celda-mes cruzada con incendio (\texttt{JOIN} + \texttt{GROUP BY anio, mes}); ranking de celdas con incendio observado y menor precipitaci\'on. Ejemplo ilustrativo (versi\'on completa en \texttt{consultas/consulta\_01\_incendios\_clima\_mensual.sql}):
...
JOIN fact_clima AS fc ON fc.celda_id = fi.celda_id AND fc.fecha_id = fi.fecha_id
JOIN dim_fecha  AS df ON df.fecha_id = fi.fecha_id
GROUP BY df.anio, df.mes, fi.celda_id;
\end{verbatim}
\end{footnotesize}

\textbf{En MongoDB:} conteo y FRP promedio/m\'aximo de detecciones v\'alidas por \texttt{celda\_id} y mes, filtrando por umbral m\'inimo de \texttt{fire\_mask}, mediante \texttt{aggregate()} con \texttt{\$match} y \texttt{\$group} (ver \texttt{nosql/consultas\_mongodb.js}).
